  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl2 border border-sand bg-white transition-shadow hover:shadow-lg">
      <div className="relative aspect-[4/5] w-full overflow-hidden">
        <Link href={`/blogg/${post.slug}`} className="absolute inset-0 z-0 block">
          <Image
            src={post.coverImage}
            alt={post.title}
            fill
        ...
        />
        <span className="pointer-events-none absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-sage-dark">
          {post.category}
        </span>
      </div>
